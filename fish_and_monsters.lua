...
local blatantDebugLogs = {}
local function bLog(str)
    local formatted = string.format("[%s] %s", os.date("%X"), str)
    table.insert(blatantDebugLogs, formatted)
    if #blatantDebugLogs > 150 then table.remove(blatantDebugLogs, 1) end
    print(formatted)
end
...
-- Menggunakan loop background VirtualUser yang 100% bebas lag di mobile executor
local function toggleAntiAFK(state)
    antiAFK = state
    print("[F&M] Anti-AFK set to: " .. tostring(state))
end

task.spawn(function()
    while true do
        task.wait(120) -- Simulasi ketukan setiap 2 menit sekali untuk mencegah 20 menit idle
        if antiAFK then
            pcall(function()
    ...
    pcall(function()
        for _, gui in ipairs(LP.PlayerGui:GetChildren()) do
            if gui:IsA("ScreenGui") and gui.Enabled then
                ...
                -- Hanya scan gui bertema fishing / reward / minigame agar tidak lag
                if gn:find("fish") or gn:find("minigame") or gn:find("reward") or gn:find("catch") or gn:find("hud") or gn:find("play") or gn:find("alert") then
                    for _, child in ipairs(gui:GetDescendants()) do
                        if child:IsA("TextButton") or child:IsA("ImageButton") then
                            local text = ""
                            pcall(function() text = child.Text:lower() end)
                            local cn = child.Name:lower()
                            if text:find("continue") or text:find("tap") or text:find("buka") or text:find("close") or text:find("skip") or
                               cn:find("continue") or cn:find("close") or cn:find("dismiss") or cn:find("skip") then
                                pcall(function()
                                    child:Activate()
                                    if typeof(firesignal) == "function" then
                                        pcall(firesignal, child.MouseButton1Click)
                                        pcall(firesignal, child.Activated)
                                    else
                                        local events = {"MouseButton1Click", "MouseButton1Down", "TouchTap", "Activated"}
                                        for _, ev in ipairs(events) do
                                            pcall(function()
                                                local evSignal = child[ev]
                                                if evSignal and typeof(getconnections) == "function" then
                                                    for _, con in ipairs(getconnections(evSignal)) do
                                                        pcall(con.Function)
                                                    end
                                                end
                                            end)
...
TabFishing:CreateButton({
    ...
    Callback = function()
        ...
        log("\n[6] BLATANT FISHING EXECUTION LOGS:")
        if #blatantDebugLogs == 0 then
            log("  No blatant fishing execution logs recorded yet. Turn on Blatant Fishing first!")
        else
            for _, line in ipairs(blatantDebugLogs) do
                log("  " .. line)
            end
        end
...
local function runBlatantFishingCycle()
    ...
    if not (ThrowFloater and ConfirmFloatingCast and StartPulling and StopFishing and FishingPullInput) then
        bLog("ERROR: Missing core remotes!")
        return false
    end
    ...
    local r1Ok, r1Err = pcall(function() StopFishing:InvokeServer() end)
    bLog("StopFishing (reset) sent. success=" .. tostring(r1Ok) .. " err=" .. tostring(r1Err))
    task.wait(0.1)

    if StartFishing then
        local r2Ok, r2Err = pcall(function() StartFishing:InvokeServer(activeRodName, activeFloaterName) end)
        bLog("StartFishing sent. success=" .. tostring(r2Ok) .. " err=" .. tostring(r2Err))
    end

    local char = LP.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then bLog("ERROR: Character or HRP missing!") return false end

    local origin = hrp.Position
    local target = getWaterTarget(origin)
    bLog("Target coordinates: " .. tostring(target))
    ...
    bLog("Current FishingCastId: " .. tostring(oldCastId))
        ...
        connections[#connections+1] = FishCaught.OnClientEvent:Connect(function(...)
            for _, v in ipairs({...}) do
                if type(v) == "string" and v ~= "" and not v:match("^%x+-%x+-%x+-%x+-%x+$") then
                    caughtFishName = v
                    bLog("Event FishCaught received: " .. tostring(caughtFishName))
                end
                if type(v) == "table" then
                    local n = extractFishName(v)
                    if n then 
                        caughtFishName = n 
                        bLog("Event FishCaught (table) parsed: " .. tostring(caughtFishName))
                    end
                end
            end
        end)
    ...
    if FishingSuccess and FishingSuccess:IsA("RemoteEvent") then
        connections[#connections+1] = FishingSuccess.OnClientEvent:Connect(function(...)
            for _, v in ipairs({...}) do
                if type(v) == "string" and v ~= "" and not v:match("^%x+-%x+-%x+-%x+-%x+$") then
                    caughtFishName = v
                    bLog("Event FishingSuccess received: " .. tostring(caughtFishName))
                end
                if type(v) == "table" then
                    local n = extractFishName(v)
                    if n then 
                        caughtFishName = n 
                        bLog("Event FishingSuccess (table) parsed: " .. tostring(caughtFishName))
                    end
                end
            end
        end)
    ...
    if FishingPullState and FishingPullState:IsA("RemoteEvent") then
        connections[#connections+1] = FishingPullState.OnClientEvent:Connect(function(...)
            ...
            bLog("Event FishingPullState received!")
        end)
    end

    -- 1. Throw Floater
    local r3Ok, r3Err = pcall(function() ThrowFloater:InvokeServer(origin, target, activeRodName, activeFloaterName, floatConfig, 2.5) end)
    bLog("ThrowFloater sent. success=" .. tostring(r3Ok) .. " err=" .. tostring(r3Err))
    task.wait(0.2)

    -- 2. Confirm Cast
    local r4Ok, r4Err = pcall(function() ConfirmFloatingCast:InvokeServer(target) end)
    bLog("ConfirmFloatingCast sent. success=" .. tostring(r4Ok) .. " err=" .. tostring(r4Err))
    task.wait(0.05)
    ...
    if not uuid then
        bLog("ERROR: UUID is still NIL after fallback! Aborting.")
        disconnectAll()
        pcall(function() StopFishing:InvokeServer() end)
        return false
    end

    -- 4. Start Pulling & Begin Minigame
    ...
    -- Tunggu FishingPullState (maksimal 1.5 detik) agar server siap menerima input tap
    bLog("Waiting for pull state signal...")
    local wsrv = 0
    while wsrv < 1.5 and not serverReadyForPull do
        task.wait(0.02)
        wsrv = wsrv + 0.02
    end
    bLog("Pull state signal ready=" .. tostring(serverReadyForPull))

    -- Spam ketukan tap secara dinamis sampai ikan tertangkap (max 3.5 detik)
    bLog("Starting dynamic tapping loop...")
    local tapCount = 0
    local startTap = os.clock()
    ...
    bLog("Catch confirmed=" .. tostring(caughtFishName ~= nil) .. " fishName=" .. tostring(caughtFishName))

    disconnectAll()

    -- 5. Klaim & Hentikan Sesi
    local r7Ok, r7Err = pcall(function() StopFishing:InvokeServer() end)
    bLog("StopFishing (claim) sent. success=" .. tostring(r7Ok) .. " err=" .. tostring(r7Err))
    task.wait(0.05)

    -- Tutup banner secara instan
    dismissCaughtBanner()
    bLog("--- BLATANT CYCLE COMPLETED ---")
    return true
end
...
task.spawn(function()
    while true do
        task.wait(0.3) -- Jeda aman 300ms (Sangat hemat CPU)
...
local ISLANDS = {
    ...
    { name = "Pulau Paus yang Hilang",  locationId = "Lost Whale Island", islandModel = "Island_Whale",     hangout = "IslandWhaleHangout",  coords = Vector3.new(-2713.0, 175.6, 114.2) },
    ...
    { name = "Cape Town",               locationId = "Cape Town",        islandModel = "Seabreeze",         hangout = "Rakit_TiresHangout",  coords = Vector3.new(2382.1, 200.0, -3125.0) }, -- Cape Town koordinat kembali ke 2382, -3125 dengan Y=200 agar aman
...
local function teleportToIsland(islandData)
    ...
    if islandData.coords then
        ...
        -- ANCHOR CHARACTER SELAMA 2 DETIK agar map / pulau ter-stream oleh StreamingEnabled
        -- Ini mencegah player jatuh menembus lantai (Volcano / Bora Reef)
        hrp.Anchored = true
        task.spawn(function()
            task.wait(2.2)
            hrp.Anchored = false
            print("[F&M Island] Character unanchored after load.")
        end)
    ...
    return false, "❌ Gagal: " .. islandData.name .. " - semua metode gagal!"
end
...
for _, island in ipairs(ISLANDS) do
    ...
    TabPlayer:CreateButton({
        Name = "⛵ " .. island.name,
        Callback = function()
            local ok, msg = teleportToIsland(islandCopy)
            Rayfield:Notify({
                Title = ok and "✅ Teleport!" or "❌ Gagal",
                Content = msg,
                Duration = 4
            })
        end
...
TabPlayer:CreateButton({
    ...
    Callback = function()
        ...
        -- Isi Folder Location (RECURSIVE)
        local locFolder = workspace:FindFirstChild("Location")
        if locFolder then
            print("--- workspace.Location descendants (Coordinates) ---")
            for _, islandFolder in ipairs(locFolder:GetChildren()) do
                local foundPos = nil
                ...
                print(string.format("  Location Folder: %s => Coords: %s", islandFolder.Name, tostring(foundPos or "Not Found")))
            end
        else
            print("  [Location folder NOT FOUND in workspace]")
        end
        -- Semua StoryNPC models
        local storyNpc = workspace:FindFirstChild("StoryNPC")
        if storyNpc then
            print("--- StoryNPC children ---")
            for _, child in ipairs(storyNpc:GetChildren()) do
                local foundPos = nil
                pcall(function()
                    local bp = findBasePart(child)
                    if bp then foundPos = bp.Position end
                end)
                print(string.format("  StoryNPC child: %s => Coords: %s", child.Name, tostring(foundPos or "Not Found")))
            end
        end
        ...
        print("=== END SCAN ===")
        Rayfield:Notify({Title = "Scan Done!", Content = "Cek console F9 untuk detail!", Duration = 5})
    end
})
...
-- Helper: trigger ProximityPrompt (Redundant simulation)
local function firePrompt(prompt)
    if not prompt or not prompt.Parent then return end
    pcall(function()
        -- 1. Gunakan standard executor function
        if typeof(fireproximityprompt) == "function" then
            fireproximityprompt(prompt)
        end
        -- 2. Backup simulation (InputHold)
        task.spawn(function()
    ...
    Callback = function()
        ...
        for _, desc in ipairs(workspace:GetDescendants()) do
            if desc:IsA("ProximityPrompt") then
                ...
                local parentName = parent and parent.Name or "No Parent"
                local parentClass = parent and parent.ClassName or "None"
                local parentPos = "N/A"
                if parent and parent:IsA("BasePart") then
                    parentPos = tostring(parent.Position)
        ...
        for _, desc in ipairs(workspace:GetDescendants()) do
            local nm = desc.Name:lower()
            if nm:find("chest") or nm:find("treasure") or nm:find("peti") then
